...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        coords_h = torch.arange(self.window_size, device=x.device)
        coords_w = torch.arange(self.window_size, device=x.device)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x_view = F.pad(x_view, (0, 0, pad_l, pad_r, pad_t, pad_b))
        _, Hp, Wp, _ = x_view.shape
...
class PiDiNet(nn.Module):
    def __init__(self, in_chans=14):
        super().__init__()
        self.backbone = nn.Sequential(
            nn.Conv2d(in_chans, 56, kernel_size=3, padding=1),
            nn.BatchNorm2d(56),
...
class LCAF(nn.Module):
    def __init__(self, dim, window_size=7, num_heads=4):
        ...
        self.proj_out = nn.Sequential(nn.Linear(dim, dim), nn.Dropout(0.1))
        self.norm = nn.LayerNorm(dim)

    def forward(self, edge, body):
        B, C, H, W = edge.shape
        edge_tokens = edge.flatten(2).transpose(1, 2)
        body_tokens = body.flatten(2).transpose(1, 2)

        edge_tokens = self.norm(edge_tokens)
        body_tokens = self.norm(body_tokens)

        q = self.proj_q(edge_tokens)
        k = self.proj_k(body_tokens)
        v = self.proj_v(body_tokens)
        ...
        q_map = q.permute(0, 2, 3, 1).reshape(B, H, W, C)
        k_map = k.permute(0, 2, 3, 1).reshape(B, H, W, C)
        v_map = v.permute(0, 2, 3, 1).reshape(B, H, W, C)

        q_map = F.pad(q_map, (0, 0, 0, pad_w, 0, pad_h))
        k_map = F.pad(k_map, (0, 0, 0, pad_w, 0, pad_h))
        v_map = F.pad(v_map, (0, 0, 0, pad_w, 0, pad_h))

        _, Hp, Wp, _ = q_map.shape

        def window_partition(x_map):
            xw = x_map.view(B, Hp // win, win, Wp // win, win, C)
            xw = xw.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, win * win, C)
            xw = xw.view(-1, win * win, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            return xw

        q_w = window_partition(q_map)
        k_w = window_partition(k_map)
        v_w = window_partition(v_map)

        attn = (q_w @ k_w.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)

        out = (attn @ v_w).transpose(1, 2).reshape(-1, win * win, C)
        out = out.view(B, Hp // win, Wp // win, win, win, C).permute(0, 1, 3, 2, 4, 5).contiguous()
...
class LERAFormernolsk(nn.Module):
    def __init__(self, in_chans=14, num_classes=2, use_dger=False):
        ...
        self.edge_encoder = PiDiNet(in_chans=in_chans)
        self.lcafs = nn.ModuleList([LCAF(56), LCAF(112), LCAF(224), LCAF(448)])
        ...
        for i in range(4):
            ...
            if self.use_dger:
                with torch.no_grad():
                    ...
                    disagree = (pseudo_body != pseudo_edge).float().unsqueeze(1)
...
if __name__ == '__main__':
    model = LERAFormernolsk(in_chans=14, num_classes=2, use_dger=False)
