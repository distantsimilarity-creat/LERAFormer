...
class LCAF(nn.Module):
    ...
    def forward(self, edge, body):
        ...
        edge = self.norm(edge.float())
        body = self.norm(body.float())
        q = self.proj_q(edge)
        k = self.proj_k(body)
        v = self.proj_v(body)
        ...
        return out.to(orig_dtype) + edge.to(orig_dtype).transpose(1, 2).reshape(B, C, H, W)

class LERAFormernoCECA(nn.Module):
    def __init__(self, in_chans=14, num_classes=2, use_dger=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            edge_input = edge_reprogrammed
            body_input = body_feat
...
if __name__ == '__main__':
    model = LERAFormernoCECA(in_chans=14, num_classes=2, use_dger=True)
