...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        shifted_x = attn_windows.view(B, Hp // self.window_size, Wp // self.window_size, self.window_size,
                                      self.window_size, C)
...
class PiDiNet(nn.Module):
    def __init__(self):
        ...
        self.backbone = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),
            nn.Conv2d(224, 448, kernel_size=3, padding=1),  # 224->448
            nn.BatchNorm2d(448),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),
        )
...
class LERAFormerNoLERA(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            fused_feat = self.lcafs[i](edge_feat, body_feat)
...
if __name__ == '__main__':
    model = LERAFormerNoLERA(in_chans=14, num_classes=2)
